...
const FilterSection = ({ title, options, selected, onChange }) => {
    ...
    return (
        <div className="border-b border-slate-800 py-4 last:border-0">
            <button onClick={() => setIsOpen(!isOpen)} className="flex items-center justify-between w-full text-left mb-2 group">
...
const RangeFilter = ({ label, min, max, onMinChange, onMaxChange }) => (
    <div className="border-b border-slate-800 py-4">
        <p className="font-semibold text-slate-200 mb-3">{label}</p>
        <div className="flex items-center gap-2">
            <div className="flex-1">
                <label className="text-xs text-slate-500 mb-1 block">Min (₹ Lakhs)</label>
                <input
                    type="number"
                    min={0}
                    max={50}
            ...
            <div className="flex-1">
                <label className="text-xs text-slate-500 mb-1 block">Max (₹ Lakhs)</label>
                <input
                    type="number"
                    min={0}
                    max={50}
...
export default function FilterSidebar({ filters, onFilterChange, isOpen, onClose }) {
    ...
    const hasActiveFilters = Object.entries(filters).some(([k, v]) => {
        if (k === 'feesMin' || k === 'feesMax') return v > 0;
                ...
                <div className="p-6 space-y-1">
                    ...
                    <div className="hidden lg:flex items-center justify-between mb-4">
                        <h2 className="text-lg font-bold text-white flex items-center gap-2">
                            <span className="w-1 h-6 bg-red-600 rounded-full" />Filters
                        </h2>
                        {hasActiveFilters && (
                            <button onClick={() => onFilterChange('reset')} className="text-xs text-red-400 hover:text-red-300 font-medium">
                                Clear All
                            </button>
                        )}
                    </div>
